...
export default function Profile({ params }: {
    ...
    return (
        <div className="space-y-5 min-h-screen">
            ...
            <ContactSettings contact={contact} email={email} color={color} />
            <TransactionHistory contact={contact} transactions={related} />

            <Collapsible className="bg-white rounded-3xl border border-red-100 shadow-sm p-5">
                <CollapsibleTrigger className="w-full text-red-400">
                    <div className="flex justify-between items-center pr-4">
                        <h2 className="text-sm font-semibold uppercase tracking-wider">Delete Profile</h2>
                        <ChevronDown size={16}/>
                    </div>
                </CollapsibleTrigger>
                 <CollapsibleContent>
                    <DeleteContactModal onDelete={onDelete} />
                 </CollapsibleContent>
            </Collapsible>
